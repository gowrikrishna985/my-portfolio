...
(function(){
  ...
  function draw(){
    ...
    signals.forEach(sig => {
      ...
      ctx.fillStyle = 'rgba(0,240,255,0.9)'; ctx.shadowBlur = 14; ctx.shadowColor = '#00f0ff';
    ...
    nodes.forEach(n => {
      ...
      ctx.beginPath(); ctx.arc(n.x, n.y, ringR, 0, Math.PI * 2); ctx.fillStyle = grad; ctx.fill();
      ctx.beginPath(); ctx.arc(n.x, n.y, 5 + activeBoost * 3, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(${baseColor},${0.8 + activeBoost * 0.2})`;
      ctx.shadowBlur = 12 + activeBoost * 10; ctx.shadowColor = `rgba(${baseColor},1)`;
      ctx.fill(); ctx.shadowBlur = 0;
    });
    const W = canvas.width, H = canvas.height;
    ...
    layerLabels.forEach((label, l) => {
      const x = padX + (l / (cols - 1)) * (W - padX * 2);
      ctx.fillStyle = 'rgba(100,116,139,0.7)'; ctx.font = '10px Space Mono, monospace';
      ctx.textAlign = 'center'; ctx.fillText(label, x, H - 12);
    });
    advanceSignals();
